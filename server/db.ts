...
export async function createIntroductionRequest(data: Omit<InsertIntroductionRequest, "id" | "createdAt" | "status">) {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");
  const [result] = await db.insert(introductionRequests).values({ ...data, status: "new" });
  return (result as any).insertId as number;
}

export async function getIntroductionRequestsByTradesperson(tradespersonId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(introductionRequests).where(eq(introductionRequests.tradespersonId, tradespersonId)).orderBy(desc(introductionRequests.createdAt)).limit(100);
}

export async function getAdminIntroductionRequests(limit = 100) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(introductionRequests).orderBy(desc(introductionRequests.createdAt)).limit(limit);
}

export async function updateIntroductionRequestStatus(id: number, status: string) {
  const db = await getDb();
  if (!db) return null;
  await db.update(introductionRequests).set({ status }).where(eq(introductionRequests.id, id));
  return { id, status };
}

// ─── Project Templates ──────────────────────────────────────────────────────

export async function createProjectTemplate(data: Omit<InsertProjectTemplate, "id" | "createdAt" | "updatedAt">) {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");
...
export async function getProjectTemplatesByUser(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(projectTemplates).where(eq(projectTemplates.userId, userId)).orderBy(desc(projectTemplates.updatedAt)).limit(50);
}

export async function deleteProjectTemplate(id: number, userId: number) {
  const db = await getDb();
  if (!db) return;
  await db.delete(projectTemplates).where(and(eq(projectTemplates.id, id), eq(projectTemplates.userId, userId)));
}
