...
async function sendEmail(to: string, from: string, subject: string, body: string): Promise<boolean> {
  ...
  if (!transporter) {
    console.warn("[Email] SMTP credentials not configured â skipping email.");
    return false;
  }
  try {
...
/** Waitlist welcome â sent when someone joins the waitlist */
export async function sendUserConfirmationEmail(toEmail: string): Promise<void> {
  const subject = "You're in â welcome to Renolab";
  const body = `Hey â thanks for joining the Renolab waitlist.
...
In the meantime go try the free estimate tool at ${APP_URL} â it's live right now and completely free to use.

Darren â Founder, Renolab`;

  await sendEmail(toEmail, fromDarren(), subject, body);
}

/** Quote request confirmation â sent to the person who submitted a formal quote request */
...
/** Estimate follow-up â sent after someone completes a guest estimate */
...
Want the full materials list, PDF export, and supplier discounts? Upgrade to Pro from just Â£9.99/month.

Need a formal quote for fitted furniture or a kitchen? Use our kitchen estimator at ${APP_URL}/kitchen-estimator â it's free to try.
...
/** Trade application confirmation â sent when a tradesperson applies */
